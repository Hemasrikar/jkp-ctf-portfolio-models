...
Each stock is encoded from its characteristics and mixed with a pooled cross
sectional context, so stocks inform one another before being scored. The network
is trained under a maximum Sharpe ratio regression objective on a rolling sixty
month window, with per seed warm starting and seed averaging.

Traded weights are the L1 normalised scores with the component lying along market
beta projected out. The projection excludes an intercept, so the portfolio's net
exposure is preserved while the residual market beta is removed. Beta is taken
from beta_60m at the formation date.
"""
...
def rolling_backtest(train_months, month_x, month_ids, month_mask, month_r, month_beta, n_features, device):
	...
	for m_idx, eom in enumerate(all_months):
		...
		w_avg = neutralise_beta(w_sum / n_valid, month_beta.get(eom))
