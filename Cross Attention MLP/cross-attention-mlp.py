"""Cross sectional attention model for CTF portfolio construction.

This is the cross sectional MLP with its mean pooled context replaced by
self attention across the stock dimension, so each stock builds its own
context from the others rather than receiving one shared average. Everything
else, the encoder, the MSRR objective, the rolling window with per seed warm
starting, and seed averaging, is unchanged. The end of run reports the realised
Sharpe on the observed return months, unscaled and scaled to a ten percent
...
class attn_net(nn.Module):
	def __init__(self, n_features):
		...
		self.head = nn.Sequential(nn.Linear(d_model, d_hidden), nn.GELU(), nn.Linear(d_hidden, 1))

	def score(self, x):
		z = self.encoder(x)
		# self attention across the stock dimension, one month is one sequence
		h = self.attn_norm(z).unsqueeze(0)
...
def rolling_backtest(train_months, month_x, month_ids, month_mask, month_r, n_features, device):
	...
	for m_idx, eom in enumerate(all_months):
		...
		for s in range(n_seeds):
			seed_all(s * 10000 + 42)
			model = attn_net(n_features).to(device)
